...
void TrafficFSM::setupStates() {
    ...
    states.append(new TrafficState("NS_Green", 10000, [=]() {
        for (auto* light : nsLights) { light->currentColor = Qt::green; light->update(); }
        for (auto* light : ewLights) { light->currentColor = Qt::red; light->update(); }

        // Left/Right pedestrian lights go green (crossing EW)
        for (auto* light : pedNS) { light->currentColor = Qt::green; light->update(); }
        for (auto* light : pedEW) { light->currentColor = Qt::red; light->update(); }
    }));

    states.append(new TrafficState("NS_Yellow", 2000, [=]() {
        for (auto* light : nsLights) { light->currentColor = Qt::yellow; light->update(); }
        for (auto* light : pedNS) { light->currentColor = Qt::red; light->update(); } // yellow = stop crossing
    }));

    states.append(new TrafficState("All_Red1", 1000, [=]() {
        for (auto* light : nsLights) { light->currentColor = Qt::red; light->update(); }
    }));

    states.append(new TrafficState("EW_Green", 10000, [=]() {
        for (auto* light : ewLights) { light->currentColor = Qt::green; light->update(); }
        for (auto* light : nsLights) { light->currentColor = Qt::red; light->update(); }

        // Top/Bottom pedestrian lights go green (crossing NS)
        for (auto* light : pedEW) { light->currentColor = Qt::green; light->update(); }
        for (auto* light : pedNS) { light->currentColor = Qt::red; light->update(); }
    }));

    states.append(new TrafficState("EW_Yellow", 2000, [=]() {
        for (auto* light : ewLights) { light->currentColor = Qt::yellow; light->update(); }
        for (auto* light : pedEW) { light->currentColor = Qt::red; light->update(); }
    }));
...
void TrafficFSM::start() {
    ...
    if (states[currentIndex]->name == "NS_Green" || states[currentIndex]->name == "EW_Green") {
...
void TrafficFSM::transitionToNextState() {
    ...
    if (nextState->name == "NS_Green") {
        bool requested = std::any_of(pedEW.begin(), pedEW.end(),
        ...
        if (requested) {
            ...
            for (auto* l : pedEW) l->buttonPressed = false;
        }
    }

    if (nextState->name == "EW_Green") {
        bool requested = std::any_of(pedNS.begin(), pedNS.end(),
                                     [](TrafficLightItem* l) { return l->buttonPressed; });
        if (requested) {
            qDebug() << "Pedestrian requested NS crossing. Reducing EW green time.";
            duration = std::min(duration, 5000);
            for (auto* l : pedNS) l->buttonPressed = false;
        }
    }
...
void TrafficFSM::checkPedestrianRequestDuringGreen() {
    if (states[currentIndex]->name == "NS_Green") {
        ...
        if (requested && secondsRemaining > 5) {
            qDebug() << "Pedestrian requested EW crossing mid-cycle. Reducing NS green duration.";
            secondsRemaining = 5;
            updateCountdownLabel();
            timer.start(5000);
        ...
        if (requested && secondsRemaining > 5) {
            qDebug() << "Pedestrian requested NS crossing mid-cycle. Reducing EW green duration.";
            secondsRemaining = 5;
            updateCountdownLabel();
            timer.start(5000);
            countdownTimer.start(1000); // reset countdown
